...
# 健康检查
curl -sf http://localhost:8080/api/    # 经 nginx 分流到 gateway
curl -sf http://localhost:8080/       # 经 nginx 分流到 web
...
```bash
./deploy/scripts/verify-split.sh http://localhost:8080/api/ 200
# 期望：C1 ≈ 5%±容差、C2 ≈ 25%±容差
```
