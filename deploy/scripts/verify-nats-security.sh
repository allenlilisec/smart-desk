...
if docker compose -f "$COMPOSE_FILE" run --rm --no-deps nats-init \
  nats --server "tls://nats:4222" --tlsca /etc/nats/certs/ca.crt pub smartdesk.ticket.created '{}' 2>/dev/null; then
...
docker compose -f "$COMPOSE_FILE" run --rm --no-deps nats-init \
  nats --server "tls://core:${NATS_CORE_PASSWORD}@nats:4222" \
...
if docker compose -f "$COMPOSE_FILE" run --rm --no-deps nats-init \
  nats --server "tls://insight:${NATS_INSIGHT_PASSWORD}@nats:4222" \
  --tlsca /etc/nats/certs/ca.crt \
...
docker compose -f "$COMPOSE_FILE" run --rm --no-deps nats-init \
  sh -c "nats --server tls://insight:${NATS_INSIGHT_PASSWORD}@nats:4222 --tlsca /etc/nats/certs/ca.crt sub smartdesk.ticket.created --count 1 --timeout 3s >/dev/null"
