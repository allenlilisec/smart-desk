...
| 任务 | 价值 | 验收 |
|---|---|---|
| 建立 Tailwind 主题令牌与语义色映射 | 避免并行页面风格分裂 | `tokens.css` 或 Tailwind theme 中包含色彩、间距、圆角、字体令牌。 |
| 建立基础业务组件 | 统一状态、优先级、SLA、空态、错误态 | Story 或组件示例覆盖所有状态；组件支持键盘与 ARIA。 |
| 确定坐席三栏工作台壳 | 决定后续页面骨架 | 宽屏三栏、窄屏深链/抽屉规则明确。 |
| 建立统一反馈系统 | 降低首版“坏页面”感 | 401/403/429、网络错误、离线、慢响应都有统一组件。 |

### 7.2 P1：WEB-1/WEB-2 首批体验增益

| 任务 | 价值 | 验收 |
|---|---|---|
| 报单向导优化 | 降低提单门槛 | 分步、草稿、附件失败不丢字段、提交成功页明确下一步。 |
| 队列默认视图 | 提升坐席处理效率 | 我的待处理、待分派、SLA 风险、超时、待客户响应可切换。 |
| 工单详情 header 固定区 | 快速理解当前工单 | 状态、优先级、SLA、负责人、客户、下一步动作同屏可见。 |
| 右侧上下文面板 | 支撑智能辅助与历史关联 | 客户信息、最近工单、相似工单、AI 建议至少有占位与降级态。 |
...
| 危险配置二次确认 | 降低误操作 | 删除/停用/批量变更有确认与影响范围。 |
| 看板指标解释 | 让图表可行动 | 每张图有口径、时间范围、生成时间、空态与导出。 |
| 图表色规范 | 避免状态色滥用 | 图表色与工单状态色不冲突，满足对比度。 |

### 7.4 P3：效率增强

| 任务 | 价值 | 验收 |
|---|---|---|
| 命令菜单 | 降低高频操作路径 | 支持创建工单、切换队列、分派、流转、搜索。 |
| 快捷键体系 | 提升坐席效率 | `?` 查看快捷键；常用键不抢输入框焦点。 |
| 批量操作 | 处理积压 | 队列多选后支持批量分派、标记、关闭/暂停等受权限控制操作。 |
| 坐席冲突提示 | 避免重复回复 | 有其他坐席查看/编辑/回复时展示提示；后端能力未就绪则先预留 UI。 |

## 8. 评审待决事项

需架构设计团队裁决：

1. 是否确认“坐席工作台三栏壳”为 WEB-2 标准布局：队列、主体、上下文/AI 辅助。
2. 是否将设计令牌与基础组件纳入 WEB-0 验收门禁，而不是 WEB-1/2 页面内各自实现。
3. 是否允许首版预留命令菜单/快捷键 API 与组件接口，但 P3 再完整实现。
4. 坐席冲突提示是否需要 gateway/core 支撑实时占用状态，若不进 M2，则 UI 先展示静态预留还是完全延后。
5. 看板图表库是否按 web 详设倾向采用 Recharts，并同步定义图表色板。

需产品业务团队输入：

1. requester 门户的提单分类、字段顺序、默认 SLA 预期文案。
2. 坐席队列默认视图名称与排序口径。
3. 管理后台配置项的危险操作清单。
4. 看板首批指标优先级：SLA 达成、积压、工作量、分类分布、重开率、CSAT。
