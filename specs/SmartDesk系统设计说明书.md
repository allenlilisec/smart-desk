# SmartDesk 智能服务平台 — 系统设计说明书（M1·整体大设计）
...
**待评审/确认事项**：
1. 配置归属决策（taxonomy/SLA 策略归 core、通知策略归 insight、认证凭证归 gateway/角色主数据归 core）——请 CTO 确认。
2. 事件总线选型 NATS JetStream——请 CTO 确认（RabbitMQ 为等价备选）。
3. 检索一期实现（PG 全文 vs OpenSearch）——契约已前向兼容，实现选型可在 M3 详设时再定。
4. OQ-10 合规（留存期/法务判定）——保留人类/法务在 M4 GA 前闭环，不阻塞 M1/M2。
