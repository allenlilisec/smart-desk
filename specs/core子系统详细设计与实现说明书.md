...
> **引用层级**：`src/openapi/core.yaml`（契约唯一事实源）→ [`SmartDesk系统详细设计与实现说明书.md`](SmartDesk系统详细设计与实现说明书.md)（系统级事实源）→ 本文（core 子系统实现级派生）→ [`tasks.md`](core/tasks.md)（执行清单）→ [`lifecycle-done-gap-drift.md`](core/lifecycle-done-gap-drift.md)（实现快照）。下游不得突破上游约定。
...
| 术语 | 含义 | 备注 |
|---|---|---|
| service-jwt / serviceAuth | gateway 签发、aud=core 的服务令牌；core 验签后从中读取身份 claims | core.yaml `security: serviceAuth` 即此机制 |
| claims | service-jwt payload 中的 `sub`（用户 ID）、`roles`（角色数组）、`org_id`（租户 ID） | 旧 `X-User-* / X-Org-Id` 明文透传头已废弃 |
| outbox | 事务性发件箱：业务写库与事件落表同一事务，再由 relay 异步投递 | MVP 当前为 in-memory best-effort，生产前升级为事务性 outbox + NATS relay |
| MVP | 当前 `src/smartdesk-core/` 的轻量实现：`httpapi/domain/store` 三件套，手写 `net/http` | 原六边形/sqlc/oapi-codegen 目标架构已归档为长期技术债（[SUP-298](mention://issue/c7b6eec2-990f-4491-ad9b-33cb47ac8151)） |
| done / gap / drift | 资料一致性分类：done=已就绪、gap=已知缺口、drift=口径/实现偏离需对齐 | 详见 `lifecycle-done-gap-drift.md` |
...
> **MVP 实际映射（D-2 裁决）**：当前 `src/smartdesk-core/` 采用 `internal/httpapi` + `internal/domain` + `internal/store` 轻量结构，尚未按上图拆分为 `ticket/transition/assignment/sla/comment/attachment/link/timeline/config/repository/events/objstore/platform` 等子包，也未使用 `oapi-codegen`/`sqlc`。MVP 为新事实源，目标六边形架构及生成工具已归档为长期技术债（[SUP-298](mention://issue/c7b6eec2-990f-4491-ad9b-33cb47ac8151)）。
>
> | 目标架构层/包 | 当前 MVP 映射 | 说明 |
> |---|---|---|
> | 入站适配器 `httpapi` + `middleware` | `internal/httpapi/server.go` + `auth.go` | 手写 `net/http` + method/path patterns；中间件链内联在 `Server` 中 |
> | 应用/领域层 `ticket/transition/assignment/sla/comment/attachment/link/timeline/config` | `internal/domain`（状态机、SLA、可见性）+ `internal/httpapi/ticket_handlers.go`/`workflow_handlers.go`/`collaboration.go`/`attachments.go`/`config_handlers.go` | 领域规则已抽出；业务 handler 仍与 HTTP 适配器同包 |
> | 出站端口 `repository` + `events` + `objstore` | `internal/store`（`memory.go`/`postgres.go` 双实现 + `memory_collaboration.go`/`postgres_*.go`）+ `internal/event` | 无 sqlc queries，无独立 objstore 包；事件为 in-memory best-effort |
> | 平台层 `platform` | `internal/platform` 未单独成包；DB/配置/健康检查分散在 `store`、`httpapi`、`cmd` | `/readyz` 不强制探测 DB/NATS（D-4 裁决，[SUP-297](mention://issue/d2a2e719-53ad-432e-b0bf-a51cdb0cf37d)） |

### 2.2 建单请求处理链（落地系统详设 §9 主流程 step 2）

```
POST /v1/tickets  (Idempotency-Key)
 1. recover → requestID(透传 X-Request-Id，仅用于链路追踪) → serviceAuth(校验 service-jwt 签名 + aud=core)
 2. userCtx：从已验签的 service-jwt claims 提取 sub / roles / org_id 注入 context
...
> **MVP 事实标注（D-3 裁决）**：当前实现为 `internal/event` in-memory best-effort 发布，无事务性 `outbox_events` 表、无 relay、无 NATS JetStream。生产前必须升级为「业务表 + timeline + outbox」单事务三写 + `FOR UPDATE SKIP LOCKED` relay + NATS（[SUP-296](mention://issue/c8498d4e-a331-4a39-8e43-f9944149b12c)）。
...
> **MVP 事实标注（D-5 裁决）**：当前迁移基线为 `0001_init.sql`（压缩基线）+ `0002_ticket_counter.sql` + `0003_attachments.sql` + `0005_seed.sql`，未按上表拆分为 0001–0004。`roles`、`user_roles`、`sla_policy_targets`、`ticket_status_history`、`watchers`、`csat_ratings`、`outbox_events` 等表在 MVP schema 中缺失或形态简化（如 `users.roles TEXT[]` 代替 `roles/user_roles`，`sla_policies.targets JSONB` 代替 `sla_policy_targets`）。`0001_init.sql` 已作为新事实源，后续新表走独立 migration（[SUP-256](mention://issue/6dc94180-4236-4a26-83f3-aa8770cb73ed)）。
...
-- MVP 事实标注：当前 `0001_init.sql` 中 `idempotency_keys` 表仅保留 `org_id/key/created_at` 等最小字段，
-- 未含 `request_hash/response_json/status_code`；命中同 key 直接返回首次 ticket ID，不同请求体不返回 409。
-- 该缺口已记入 `lifecycle-done-gap-drift.md` T022 gap，待后续独立 migration 补齐。
...
- 入站：仅信任 gateway 的 `serviceAuth`(service-jwt，校验签名 + `aud=core`)，mTLS 通道；最终用户身份从已验签 claims 读取（`sub/roles/org_id`）。
- core **不二次鉴权用户身份**，但用 claims 中的身份做领域级数据过滤（内部备注、本组范围、附件越权、requester 可见性）——纵深防御。
- `X-Request-Id` 仅用于链路追踪，`trace_id`/`request_id` 全链路透传并落日志与时间线；旧的 `X-User-* / X-Org-Id` 明文透传头已废弃，不再作为身份来源。
...
> **MVP 事实标注（D-3 裁决）**：当前为 `internal/event` in-memory best-effort 发布，无 `outbox_events` 表、无 NATS JetStream、无 `ticket.sla_warning`/`ticket.sla_breached`。MVP 接受该降级，生产前必须升级为事务性 outbox + NATS relay（[SUP-296](mention://issue/c8498d4e-a331-4a39-8e43-f9944149b12c)）。
...
> **当前 MVP 包映射（D-2 裁决）**：`src/smartdesk-core/` 未按上图拆分子域包，实际为 `cmd/smartdesk-core/main.go` + `internal/{auth,config,domain,event,httpapi,id,store}`。目标分包已归档为长期技术债（[SUP-298](mention://issue/c7b6eec2-990f-4491-ad9b-33cb47ac8151)）。
>
> | 目标包 | 当前 MVP 位置 | 状态 |
> |---|---|---|
> | `cmd/smartdesk-core/main.go` | `cmd/smartdesk-core/main.go` | ✅ 一致 |
> | `internal/appconfig` | 内联在 `cmd/main.go` + `internal/config` | ⚠️ 未独立成包 |
> | `internal/httpapi/{gen,middleware}` | `internal/httpapi/server.go`/`auth.go` | ⚠️ 手写路由，无 gen，中间件内联 |
> | `internal/domain` | `internal/domain` | ✅ 一致 |
> | `internal/ticket/transition/assignment/sla/comment/attachment/link/timeline/config` | 能力分散在 `internal/httpapi/ticket_handlers.go`、`workflow_handlers.go`、`collaboration.go`、`attachments.go`、`config_handlers.go` + `internal/domain` | ⚠️ 未按子域分包 |
> | `internal/repository` | `internal/store`（memory/postgres 双实现） | ⚠️ 无 sqlc/ports 抽象 |
> | `internal/events` | `internal/event` | ⚠️ in-memory best-effort |
> | `internal/objstore` | 未独立；附件 URL 为 core 内置短时签名 | ⚠️ gap |
> | `internal/platform` | 未独立；DB/健康检查分散在 `store`/`httpapi` | ⚠️ 未独立成包 |
> | `migrations/` | `migrations/0001_init.sql` + `0002_ticket_counter.sql` + `0003_attachments.sql` + `0005_seed.sql` | ⚠️ D-5 压缩基线 |
> | `test/` | `internal/httpapi/server_test.go` + `internal/store/postgres_integration_test.go` | ⚠️ 无 testcontainers |
...
注：当前 OpenAPI 为 `1.1.0`，身份来源已统一为 gateway 签发的 service-jwt claims（`sub`/`roles`/`org_id`），本文已据此刷新；`specs/core/tasks.md` 如仍有旧透传头表述，应以契约和现代码为准同步刷新，或明确要求代码回退。
...
| 依赖 | 类型 | 说明 / 阻塞点 |
|---|---|---|
| **契约冻结**（梁栋） | 前置门禁 | 系统详设 §12.3：冻结前不得编码；core 详设可在框架内并行准备。**`core.yaml info.version=1.1.0` 已冻结**（O5 裁决，PR #27 去 `-draft` 为 1.0.0，PR #33 升级为 1.1.0），开发 Issue 可解阻塞 |
| `src/openapi/core.yaml` | 契约 | 接口唯一事实源；变更经梁栋批准、秦诺 `api-contract-check` 校验 |
| **gateway**（GW-5/GW-3） | 上游调用方 | service-jwt + mTLS + claims 身份；core 只信任 gateway 签发的已验签 claims。GW-3 聚合 BFF 依赖 core 契约就绪 |
...
> O1–O6 已由架构团队（梁栋）于 2026-06-14 裁决，事实源回填见 **PR #27 / PR #33**（系统详设 §4.3/§12 + `core.yaml` 1.1.0 冻结）。本节由「开放事项」转为**裁决记录**，本文 §7/§8/§4.2 已据此收敛，无残留标红开放项。
...
| **O5** | core.yaml `info.version` 为 `1.0.0-draft` | ✅ 去 `-draft` 冻结为 **`1.0.0`**（PR #27），后经 PR #33 升级为 **`1.1.0`** | §8.2 依赖表 |
