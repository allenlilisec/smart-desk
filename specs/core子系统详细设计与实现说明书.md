...
POST /v1/tickets  (Idempotency-Key)
 ...
 2. userCtx：解析 service-jwt 已验签 claims（sub / roles / org_id）注入 context
...
- 入站：仅信任 gateway 的 `serviceAuth`(service-jwt，校验签名 + `aud=core`)，mTLS 通道；解析 service-jwt 已验签 claims（`sub`/`roles`/`org_id`）作为身份/租户来源，`X-Request-Id` 仅作链路追踪。
- core **不二次鉴权用户身份**，但用 service-jwt 已验签 claims（`sub`/`roles`/`org_id`）做领域级数据过滤（内部备注、本组范围、附件越权）——纵深防御。
...
| **gateway**（GW-5/GW-3） | 上游调用方 | 签发 service-jwt 并携带 `sub/roles/org_id` claims + mTLS；core 只信任 gateway。GW-3 聚合 BFF 依赖 core 契约就绪 |
