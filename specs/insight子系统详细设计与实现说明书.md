...
| §5 跨服务交互 | 系统详设 §6.3 已给出 D1 三条路径原则；本文件显式展开三条路径、给出 `insight.classification_suggested` 事件 payload 的扁平 `ClassificationSuggestedPayload` 结构，并补充降级策略。 |
...
**关键约束**：
- payload 必须与 `insight.yaml#/components/schemas/ClassificationSuggestedPayload` 的**扁平结构**保持一致：
  - `predicted_category_id`
  - `confidence`
  - `priority`
  - `applied`
- **禁止在事件 payload 中使用**同步预测接口的嵌套 `PredictResult` 结构；`PredictResult` 仅用于 HTTP API `/classification/predict` 响应。
- core 消费后，按 `event_id` 幂等写入工单详情 `Ticket.suggestion`（`ClassificationSuggestion` 扁平视图：category_id、confidence、priority、applied）。core 内部只做字段命名映射，不再解析嵌套结构。
...
| 事件链路 | **partial** | `app/nats_consumer.py` 消费 `smartdesk.ticket.*`，`ProcessedEvent` 幂等，`ticket.created` 发布 `insight.classification_suggested`。 | 补 `ticket_id` 顺序分区、失败状态、剩余 M4 payload 判别模型；`ticket.created` 同步维护相似索引。 |
...
| 依赖 | 来源 | 影响范围 | 当前状态 |
|---|---|---|---|
| `src/openapi/insight.yaml` 冻结 | 架构团队（梁栋/秦诺） | 全部 HTTP 接口 | 已冻结 v1.0.0，随系统详设 v1.0 生效。 |
...
> **红色警戒**：以上 O-1（core 消费侧扁平 payload 写回）为 M3 前必须闭环项；O-3（邮件网关）为 M3 投产阻塞项；其余可保留至 M4 前决策，但需架构团队给出排期。
