...
|---|---|---|---|---|
| G-1 | [SUP-113](mention://issue/63d92b24-679e-41c9-b925-b48b4157f62e) O-1 | `insight.classification_suggested` 写回事件 payload 已由 SUP-268 固化为扁平 `ClassificationSuggestedPayload` | M3 写回链路（S-03）可按 `predicted_category_id`/`confidence`/`priority`/`applied` 断言字段；HTTP `/classification/predict` 仍保留嵌套 `PredictResult` | 集成测试按事件 payload 扁平结构断言；不要复用 HTTP `PredictResult` 作为事件 schema |
...
| G-8 | [SUP-113](mention://issue/63d92b24-679e-41c9-b925-b48b4157f62e) / 契约 | `insight.yaml` 已由 SUP-268 正式冻结为 `v1.0.0` | M3 智能/通知/统计用例已有稳定事实源 | 冻结后继续执行 `api-contract-check` 作为集成测试入口门禁 |
| G-9 | [SUP-113](mention://issue/63d92b24-679e-41c9-b925-b48b4157f62e) / 契约 | `insight.yaml#/DomainEvent` payload 中仅 `ticket.commented`/`ticket.merged` 仍标记为"M4 固化前不约束"的自由 object；`insight.classification_suggested` 已固化 | S-19 事件可靠性用例可对 `insight.classification_suggested` 做 schema 断言，剩余两类事件仍需 M4 固化 | M4 固化前至少给出 `ticket.commented`/`ticket.merged` 示例 payload 字段清单，或补充说明消费侧最小字段校验 |
