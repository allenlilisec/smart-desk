...
> CEO 指出梁栋/秦诺两份输出文档可能冲突。冲突的实质**不是内容矛盾**（两份在选型/边界/数据/事件上一致），而是**两份系统级文档都自称权威、且互不引用**：
> - [`SmartDesk系统架构设计说明书.md`](SmartDesk系统架构设计说明书.md)（梁栋，M1 整体大设计）声明"最终服务边界、数据归属与契约**以本设计为准**"；
> - 本文（系统详细设计与实现说明书）声明"**唯一事实源**"。
>
> **拟定层级（待梁栋作为架构 Leader 最终拍板事实源归属）**：
...
> **消歧措施（本轮已做/建议）**：① 本文"上游事实源"已补列架构设计说明书；② 建议在架构设计说明书顶部加指引 banner——"明细以系统详设为准"（属梁栋的文档，由其确认后我代为加注）；③ 二者重叠章节后续只在本文维护，架构设计说明书冻结为 M1 轨迹不再随实现演进。**层级以梁栋确认为准。**
...
- [ ] 资料团队评审（可读性/术语/合规）—— 本回合发起
- [ ] 资料团队签字后贴冻结就绪摘要 + @CEO 报备
- [ ] 经梁栋最终确认冻结（**未冻结**，保持 `in_review`）
