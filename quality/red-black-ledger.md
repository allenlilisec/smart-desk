...
- **类型**：黑
- **来源**：[SUP-347](mention://issue/1302fd35-9a4e-4d33-8ced-f970b4372765) 复盘：demo 日 runtime 双切换致编码产能空窗
- **事件**：2026-06-19 demo 冲刺日，6 个 codex 编码 agent 因额度问题被批量切至 opencode，额度恢复后又切回 codex；切回后未立即派活，导致 6 个 agent 在 demo 冲刺窗口内空转约 1.5h。
- **影响范围**：6 个编码 agent（石磊、陈川、江颜、关山、连城、卫泽）、demo 冲刺产能、PMO 派活节奏。
- **根因**：
  1. 冲刺/demo 窗口缺少 runtime 迁移冻结机制，迁移决策未评估对冲刺产能的影响；
  2. 迁移完成后缺少「立即派活」闭环，PMO 未在切回后 5 分钟内检查并派出任务；
  3. 迁移前未执行 checklist，未提前验证 opencode 对 `--thinking-level` 等关键参数的兼容性，导致 cursor/kimi 迁移失败；
  4. 红黑事件台账对同类流程缺口缺少系统化记录与跟踪。
- **改进项**：
  - [SUP-348](mention://issue/b477af3a-2cab-4e8a-a1ae-b9b5f102824c)：制定 runtime 迁移治理规范（冻结窗口、5 分钟派活检查、迁移前 checklist、红黑台账治理），何明评审定稿后纳入 SUP-99 方法论库；
  - [SUP-349](mention://issue/cc7d574d-c9fa-4fa9-a568-451ad885d278)：修复 CLI `--thinking-level` 缺失，转 CTO 评估排期；
  - 本事件录入红黑事件台账，由韩衡按周跟踪改进项闭环。
- **状态**：跟踪中（治理规范草案待何明定稿，CLI 修复待 CTO 排期）
- **记录人**：韩衡
- **时间线**：
  - 2026-06-19 06:13：CEO 指示将 6 个 codex agent 切到其它 runtime。
  - 2026-06-19 06:22：程远完成第一次迁移，6 个 agent 全部切到 opencode。
  - 2026-06-19 07:19：CEO 指示 codex 额度已恢复，切回 codex。
  - 2026-06-19 07:23：石磊完成切回，6 个 agent 回到 codex。
  - 2026-06-19 08:43：CEO 发现看板仍有大量工作未完成，要求程远尽快布置。
  - 2026-06-19 08:48：程远完成全量派活，6 个 agent 开始投入 demo 任务。
  - 2026-06-19 10:14：韩衡输出 SUP-347 复盘报告并提交何明评审。
  - 2026-06-19 10:20：何明评审结论：5 条改进建议原则采纳，其中 4 条纳入 SUP-99 方法论库，CLI 修复转 CTO。
