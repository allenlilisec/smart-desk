...
- **纯标准库**：无第三方依赖，离线可 `go build` / `go test`（Go 1.23，net/http method+path 路由）。
...
- **持久化**：MVP 为内存实现（`internal/store`），全链路可跑可测；`migrations/0001_init.sql`
  为权威 Postgres schema，生产适配器对其落地。
...
> 为契约已定义、本次 MVP 骨架未实现的后续项，留给 CORE-B 后续补齐。
