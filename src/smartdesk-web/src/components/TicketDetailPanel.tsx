...
import type { Comment, TicketAggregate, TicketStatus, TimelineEntry, TransitionAction } from "@/lib/types";
import { STATUS_LABELS } from "@/lib/types";
import { EmptyState, StatusBadge, PriorityBadge } from "./ui";

const AGENT_ACTIONS: readonly { readonly action: TransitionAction; readonly label: string; readonly statuses: readonly TicketStatus[] }[] = [
...
const REQUESTER_ACTIONS: readonly { readonly action: TransitionAction; readonly label: string; readonly statuses: readonly TicketStatus[] }[] = [
  { action: "close", label: "确认关闭", statuses: ["resolved"] },
  { action: "reopen", label: "重开工单", statuses: ["resolved", "closed"] },
];

interface TicketDetailPanelProps {
  readonly ticket: TicketAggregate;
  readonly mode: "agent" | "requester";
  ...
  const handleTransition = async (action: TransitionAction) => {
    ...
    try {
      ...
      if (err instanceof ApiClientError && err.status === 409) {
        setConflictMessage("该工单已被他人更新，请刷新后重试。");
        return;
      }
      throw err;
    } finally {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-line bg-white px-4 py-4 sm:px-6">
        ...
        <dl className="mt-3 grid gap-2 text-xs text-slate-600 sm:grid-cols-3 lg:grid-cols-5">
          <div className="rounded-md border border-line bg-slate-50 px-3 py-2">
            <dt>负责人</dt>
            <dd className="mt-1 font-semibold text-slate-900">{ticket.assignee_id ?? "未分派"}</dd>
          </div>
          <div className="rounded-md border border-line bg-slate-50 px-3 py-2">
            <dt>客户</dt>
            <dd className="mt-1 font-semibold text-slate-900">待接入</dd>
          </div>
          <div className="rounded-md border border-line bg-slate-50 px-3 py-2">
            <dt>分类</dt>
            <dd className="mt-1 font-semibold text-slate-900">{ticket.category_id ?? "待接入"}</dd>
          </div>
          <div className="rounded-md border border-line bg-slate-50 px-3 py-2">
            <dt>SLA</dt>
            <dd className="mt-1 font-semibold text-slate-900">待接入</dd>
        ...
        {ticket.description?.trim() ? (
          ...
          <p className="mt-3 rounded-lg bg-slate-50 px-3 py-2 text-sm text-slate-600">
            报单人尚未补充详细描述，可通过评论继续确认影响范围和复现步骤。
          </p>
        )}
        {conflictMessage && (
          ...
          <div className="mt-4 flex flex-wrap gap-2 border-t border-line pt-4">
            {available.map((a) => (
              <button
                ...
                className="rounded-md bg-action px-3 py-1.5 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-50"
              >
                {a.label}
              </button>
            ))}
          ...
          <div className="space-y-3">
            {comments.length === 0 ? (
              <EmptyState
                ...
                description={
                  mode === "agent"
                    ? "发送对外评论同步进展，或切换为内部备注记录排查线索。"
                    : "坐席回复后会显示在这里，你也可以补充新的问题现象。"
                }
              />
            ) : (
              comments.map((c) => (
            ...
            {timeline.length === 0 ? (
              ...
              timeline.map((e) => (
                <li key={e.id} className="relative border-l-2 border-slate-200 pl-4">
                  ...
                  {isTicketStatus(e.payload.to) && (
                    <p className="text-xs text-slate-500">
                      → {STATUS_LABELS[e.payload.to]}
                    </p>
                  )}
